<template>
    <MainLayout>
        <template v-slot:body>
            <productModal :show-modal-create-prod="showModal" @close="closeModalCreateprod"
                @sendMessage="acceptMessage" :product-data="productData" :is-create="isCreate"/>
                <!-- confirm button -->
                <popupConfirm :isConfirm="isShowConfirm" @close="closeConfirm" :id="productId" :productData="productData" @sendMessage="acceptMessage"/>
           <div v-if="userStore().isShowStore || store.isShowStore"  >
                 <!-- description store -->
                 <section class="mt-16 mb-2 flex justify-center flex-row h-60 p-2 gap-x-2 max-md:flex-col max-md:gap-y-2 ">
                <div class="p-4 font-poppins w-full flex flex-col justify-center items-center bg-gradient-to-b from-indigo-600 to-red-800 rounded-md text-white relative">
                    <h1 class="font-bold text-center text-4xl max-sm:text-xl">{{ store.dataStore?.name ? store.dataStore?.name.toUpperCase() : userStore().dataStoreEmployee.name.toUpperCase() }}
                    </h1>
                    <p class=" text-center text-sm">
                        {{ store.dataStore?.description ? store.dataStore?.description : userStore().dataStoreEmployee.description }}
                    </p>
                    <small class="font-semibold absolute  text-white/50 bottom-2 right-2">code:#{{ store.dataStore.code ? store.dataStore?.code: userStore().dataStoreEmployee.code}}
                    </small>
                </div>
                <div class="w-full  h-full flex justify-center items-center overflow-hidden rounded-md">
                    <img :src="`${URL}source/image/${store.dataStore?.img ? store.dataStore.img : userStore().dataStoreEmployee.img }`" alt="image store" class="h-full w-full object-cover">
                </div>
            </section>
            <!-- button create -->
            <setion class="p-2">
            ...
            <section class="w-full flex justify-center items-center mt-1" v-if="infoMessage">
                <div class="w-fit p-4 rounded-md relative" :class="infoType == 'success' ? 'bg-green-200' : 'bg-red-200'">
                ...
                <div class="w-60 h-90 overflow-hidden rounded relative  shadow-md"
                    ...
                    <div class="p-2 text-sm w-full flex flex-col gap-y-3">
                        <div class="flex flex-row h-10 w-full items-center">
                            ...
                            <h4 class="font-semibold overflow-auto w-full">{{ formatterRupiah.formatPriceToIDR(product.price) }}</h4>
                        </div>
