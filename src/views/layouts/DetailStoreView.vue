<template>
  ...
  <checkoutModal :isCheckout="isCheckout" :data-product="dataProduct" @close="close" @confirmCheckout="orderProduct"
    :storeId="store.dataAllStore[0].id" />
  ...
  <section class="w-full h-full flex flex-col items-center font-poppins gap-1 p-4 ">
    ...
    <div class="h-80 max-sm:h-64 w-full overflow-hidden flex flex-col gap-1 rounded-md">
      ...
      <div class="w-full h-full rounded-md overflow-hidden">
        <div class="w-full h-full flex justify-center items-center overflow-hidden relative">
          <img v-if="store.dataAllStore[0].img" class="h-full w-full object-cover" :src="store.dataAllStore[0].img"
            alt="">
    ...
    <div class="w-full border">
      <a :href="store.dataAllStore[0].address">
        <button
          class=" bg-gradient-to-r from-blue-600 to-green-600 hover:from-green-600 hover:to-yellow-600  text-white rounded-md p-3 w-40">
          Location
        </button>
      </a>
    </div>
    <section v-if="store.currentHours.length !== 0" class="mt-5 bg-white p-4 rounded-lg shadow-md w-full">
      ...
      <ul class="divide-y divide-gray-200">
        <li v-for="(hour, index) in store.currentHours" :key="index" class="flex justify-between py-2">
          <div class="flex items-center">
            <font-awesome-icon :icon="hour.is_closed ? 'fa-solid fa-calendar-xmark' : 'fa-solid fa-clock'"
    ...
    <!-- list product -->
    <div
      v-if="store.dataAllStore[0].products !== undefined && Object.entries(store.dataAllStore[0].products).length !== 0"
      ...
      <div
        class="border h-90 w-60 max-sm:w-full overflow-hidden rounded relative bg-black/50 backdrop-blur-md text-white"
...
const getHourOp = async () => {
    await store.getHours(store.dataAllStore[0].id)
        .then(response => {
