<template>
    <nav class="h-full w-[300px]">
        <div class="h-full pt-16 pl-4 pr-4 border-r max-md:bg-black/30 max-md:backdrop-blur-sm flex flex-col justify-between">
            <ul class="flex flex-col gap-2 font-poppins text-[12px] font-semibold text-black max-md:text-slate-200  max-md:w-1/2">
                <li v-for="menu, index in menus" :key="index">
                    <router-link :to="menu.path" class="block" active-class="text-white bg-black/40 rounded" exact-active-class="text-white">
                        <p class="p-2 rounded w-full border-b hover:bg-indigo-600 hover:text-white transition-all duration-300 cursor-pointer">
                            {{ menu.name }}
                        </p>
                    </router-link>
                    <button
                        v-if="canAccessCreateWorkspace(menu)" 
...
const menus = computed(() => {
    return [
        ...
        },
        {
            ...
            data: workspace.getDataWorkspace.data ?? user.getDataWorkspace
        },
        {
            name: 'Finance',
            path: '/finance',
...
const canAccessCreateWorkspace = (menu) => {
    return menu.icon && menu.data && (!user.isEmployee || (user.isEmployee && user.userData.role === 'operator'));
}
